...
from app.models import License, TokenConsumption, TokenGrant
...
def available_tokens(db: Session, user_id: uuid.UUID, license_obj: License) -> int:
    """Available = license monthly quota + admin grants − consumptions (current JST month)."""
    ...
    return int(license_obj.monthly_quota_tokens) + int(granted_extra) - int(consumed)
