...
| **info-only (情報)** | 状況変化を知らせるだけ。読んだら消える | 橙 `#ffa500` + 小ドット (件数なし、金ドット/バッジ無し) | 対象画面を開く OR **1週間経過** |
...
| 種別 | 色 | トークン | 用途 |
|---|---|---|---|
| 要対応 (action) ベース | `#ffa500` (橙) | `notify` (`text-notify` / `bg-notify`) | テキスト・バッジ背景・アイコン stroke |
| 要対応 (action) ドット | `#ffd700` (金) | `notify.dot` (`bg-notify-dot`) | 左上の「!」相当ドット (光彩付き) |
| 情報 (info) ベース | `#ffa500` (橙) | `notify` | テキスト・小ドット。**action と同色**、形 (金ドット/件数バッジの有無) で区別 |
| dim opacity | `0.5` | — | カード全体の透過度 |
| dim text | `text-muted` (washi muted) | `muted` | ラベル文言 |

**決定 (2026-06-13):** info を当初 blue `#3b82f6` 予定としていたが、実装が橙運用で安定していたため **action と同じ橙 `#ffa500` を正**とする。action / info の区別は **色ではなく形** (action=金ドット+件数バッジ / info=小ドットのみ) で行う。
**トークン:** `notify` / `notify.dot` を `tailwind.config.ts` に定義。class で扱える箇所は `text-notify` 等を使用。**インライン `style` 背景・SVG `:stroke` バインドは raw hex (`#ffa500`/`#ffd700`) を許容** (rank/chart 色と同じ例外扱い)。
